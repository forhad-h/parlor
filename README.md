...
> **This fork adds an optional Bengali (বাংলা) voice + vision mode** — a small
> hosted Node sidecar (`service/`) swapped in behind the same WebSocket
> protocol, entirely additive to everything below. It doesn't touch the
> on-device path documented in this file — see
> [`service/README.md`](service/README.md) to run it instead.
...
> This fork's `server.py` defaults to the hosted Bengali mode described in
> [`service/README.md`](service/README.md). The command above still runs the
> original on-device path unchanged — just add `NODE_SERVICE_URL=` (empty) to
> `src/.env` first to opt out of hosted mode. See that file's Configuration
> table for the full variable reference.
